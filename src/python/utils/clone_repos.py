#!/usr/bin/env python3
"""
Clone entire repositories from the collection to local storage
Downloads full git repositories with history to NAS
"""
...
class RepoCloner:
    """Clone GitHub repositories with full git history and rate limiting"""
    
    def __init__(self, base_dir: str = "/Volumes/plex3/codebase/repos", max_workers: int = 2, github_token: str = None):
        # Try NAS first, fallback to local if NAS is slow/unavailable
        nas_dir = "/Volumes/plex3/codebase/repos"
        local_dir = "./codebase/repos"
        ...
        if base_dir == nas_dir:
            try:
                ...
                def timeout_handler(signum, frame):
                    raise TimeoutError("NAS access timeout")
...
def main():
    """Main function"""
    print("🔗 GitHub Repository Cloner with Rate Limiting")
    print("=" * 60)
    ...
    print(f"   Target directory: /Volumes/plex3/codebase/repos/")
    ...
    # Start cloning
    cloner = RepoCloner(max_workers=workers, github_token=github_token)
